...
#define USAGE								\
	...
	"Commands\n"							\
	"	-target-socket [SOCKET-NAME]	set the socket to send command to. Sockets can be found in `$XDG_RUNTIME_DIR/dwlb2/`\n"\
		    ...
		    const char *cmd, const char *data, const char *target_socket)
{
	...
	/* Send data to all dwlb2 instances */
	while ((de = readdir(dir))) {
		if (!strncmp(de->d_name, "dwlb2-", 5)) {
...
main(int argc, char **argv)
{
	...
	snprintf(socketdir, sizeof socketdir, "%s/dwlb2", xdgruntimedir);
	...
	for (uint32_t i = 0; i < 50; i++) {
		if ((sock_fd = socket(AF_UNIX, SOCK_STREAM, 1)) == -1)
			DIE("socket");
		snprintf(sock_address.sun_path, sizeof sock_address.sun_path, "%s/dwlb2-%i", socketdir, i);
		if (connect(sock_fd, (struct sockaddr *)&sock_address, sizeof sock_address) == -1) {
			found = true;
			break;
		}
